...
export default function Header({ darkMode, toggleDarkMode }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white dark:bg-[#0A0A0A] border-b border-brand-primary-200 dark:border-brand-primary-800 transition-colors duration-300">
          ...
          <nav className="hidden md:flex space-x-8">
            {navItems.map((item) => (
              <a
                key={item.name}
                href={item.href}
                className="text-brand-primary-600 dark:text-gray-400 hover:text-black dark:hover:text-white transition-colors text-sm font-medium"
              >
                {item.name}
              </a>
            ))}
          ...
          <div className="flex items-center space-x-4">
            ...
            >
              {darkMode ? (
                ...
                <svg className="w-4 h-4 text-brand-primary-600" fill="currentColor" viewBox="0 0 20 20">
